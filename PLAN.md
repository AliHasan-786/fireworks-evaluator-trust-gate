...
- [ ] Receive a completed 30-case packet from an independent reviewer who did not build the evaluator.
- [ ] Validate the returned evidence, run the trust gate, and generate the final evaluator-human report.
- [ ] Redeploy and verify the site, then finalize only resume claims traceable to merged public evidence.

## Acceptance boundary

The live comparison is complete and its model decision is `NO_DEPLOY` because neither model meets the ambiguity guardrail. The separate evaluator trust gate remains `INSUFFICIENT_EVIDENCE` until valid blind human labels exist. The next authorized input is an independently completed `artifacts/human_labeling/completed_labels.csv`; no human outcomes may be inferred or generated by the project author.
